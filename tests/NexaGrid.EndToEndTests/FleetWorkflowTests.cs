...
namespace NexaGrid.EndToEndTests;
...
public sealed class FleetWorkflowTests
{
    ...
    public async Task ControllerAgentOperationResultWorkflowCompletes()
    {
        ...
        var inventory = new HardwareInventory("Windows Simulator", "10.0", "X64", 12, 32_000, [], "0.1.0", true);
...
{
    private readonly string _dataDirectory = Path.Combine(Path.GetTempPath(), "NexaGrid.E2E", Guid.NewGuid().ToString("N"));
